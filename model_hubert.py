...
from ezspeech.models.conformer_asr import (
    Conformer,
    Conformer_self_condition,
    Conformer_self_condition_phoneme,
)
...
class HubertForCTCSelfCondBaseline(HubertForCTC):
    ...
    def freeze_base_layers(self):
        self.hubert.encoder.requires_grad = False
...
class HubertForCTCSelfCond(HubertForCTC):
    def __init__(self, config, target_lang=None):
        super().__init__(config)
        self.dropout = nn.Dropout(config.final_dropout)
        self.adapter = nn.Linear(768, 512)
        ...
        if labels is not None:
            ...
            with torch.backends.cudnn.flags(enabled=False):
                ...
                for idx, inter_softmax in zip(
                    ...
                    phoneme_inter_loss_lst.append(temp_loss)
                    writer.add_scalar(
                        f"loss phoneme at layer {str(idx)}", temp_loss, self.step
                    )
                intermediate_loss=0.7*sum(inter_loss_lst) / len(
                            inter_loss_lst
                        )+0.3*sum(phoneme_inter_loss_lst)/len(phoneme_inter_loss_lst)
                if is_inter == True:
                    if self.step < 10000:
                        alpha = self.step/50000
                        loss = (1 - alpha) * final_loss + alpha * intermediate_loss
                    else:

                        loss = 0.8 * final_loss + 0.2 * intermediate_loss
                else:
                    loss = final_loss
