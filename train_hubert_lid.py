...
from model_hubert import HubertForCTCSelfCondPhoneme,HubertForCTCMultitask
...
# train_dataset=train_dataset.filter(lambda x: x["lan_id"]=="vi")
...
def speech_file_to_array_fn(batch):
    ...
    batch["target_text_phoneme"] = (
        batch["lid_sequence"] if batch["lid_sequence"] is not None else "<unk>"
    )
...
model = HubertForCTCSelfCondPhoneme.from_pretrained(
    ...
    inter_layer=[4,6,8,10],
    phoneme_inter_layer=[2]
)
